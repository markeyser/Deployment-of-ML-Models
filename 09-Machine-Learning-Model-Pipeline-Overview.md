...
So in essence, we need to deploy an entire data and machine learning pipeline. That is why we speak of pipelines and not just of models. We need to deploy an entire pipeline, an entire sequence of steps that will allow us to get the data into where our mode is located, transform the data, field missing values and called categorical variables etc. Then select or somehow use only the variables that we agreed are the most predictive ones and, then utilize those variables to make a mathematical calculation utilized in either bespoke or off the shelf algorithms and, then be able to get those predictions out and send them back to our business systems.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2Mjk4OTQyNDUsLTE1MjAyNjIyNjMsMT
c3NTk4MjkyOF19
-->